...
OWNER_ID = 8651895707 # Main Owner ID (Updated)
...
def start_cmd(message):
    ...
    # Kono logo chara ekdom clean welcome message
...
def bottom_menu_handler(message):
    ...
    if message.text == "👤 Profile":
        ...
        for i, u in enumerate(top): 
            # Notun Leaderboard Format: Name (ID) - ROLE - Downloads
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            if dl_type == 'thumb':
                ...
                if thumb_url:
                    ...
                    if user.role != 'owner': user.daily_downloads += 1
            ...
            if os.path.exists(path):
                if os.path.getsize(path) / (1024 * 1024) > 49.5:
                    bot.send_message(call.message.chat.id, "❌ Video is larger than 50MB Telegram limit!")
                    os.remove(path)
                    return
